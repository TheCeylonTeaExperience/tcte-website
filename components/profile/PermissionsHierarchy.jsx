...
const PermissionHierarchy = forwardRef((props, ref) => {
    ...
    useEffect(() => {
        if (!props.initialPermissions) return;
        ...
        setPermissions(prev => prev.map((p) => {
            if (isParent(p)) {
                return {
                    ...
                    children: p.children.map((cp) => {
                        const perm = props.initialPermissions.find((pe) => pe.route === cp.route);
            ...
            const perm = props.initialPermissions.find((pe) => pe.route === p.route);
                    ...
                    if (isParent(permission)) {
                        return (
                            <div key={idx}>
                                <div className="flex items-center">
                                    <Input type="checkbox" id={`"${idx}"`} className="peer h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" checked={permission.children.filter((p) => p.permitted).length > 0 ? true : false} onChange={() => togglePermission(permission)} disabled={props.disabled} />
                                ...
                                <div className="ml-6 mt-2 space-y-2 border-l border-gray-200 pl-4">
                                    {permission.children.map((childPermission, idx) => (
                                        <div key={`"${idx}"`} className="flex flex-col space-y-1">
                                            <div className="flex items-center">
                                                <Input type="checkbox" id="child-1" className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" checked={childPermission.permitted} onChange={() => togglePermission(childPermission)} disabled={props.disabled} />
                        ...
                        return (
                            <div key={`"${idx}"`} className="flex flex-col">
                                <div className="flex items-center">
                                    <Input type="checkbox" id="parent" className="peer h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" checked={permission.permitted} onChange={() => togglePermission(permission)} disabled={props.disabled} />
                                ...
                                {permission.permitted && (<Select
                                    value={permission.accessType} onValueChange={(value) => handlePermissionTypeChange(permission, value)} disabled={props.disabled}>
                                    <SelectTrigger id="permissionType">
                                        <SelectValue placeholder="Select a permission type" />
                                    </SelectTrigger>
                                    <SelectContent>
